...
const getSessionExpiryDate = () => {
  ...
  const cookieExpiry = 7 * 24 * 60 * 60 * 1000; // 7 days from now

  return { tokenExpiry, cookieExpiry };
};
...
export const otpVerification = async (req, res) => {
  ...
  try {
    ...
    const { tokenExpiry, cookieExpiry } = getSessionExpiryDate();

    user.refreshToken = refreshToken;
    user.sessionExpiresAt = tokenExpiry;
    await user.save();
    ...
    res.status(200).json({ userDetail, _a_t: accessToken });
...
export const signin = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ msg: "Login Successfully", userDetail, _a_t: accessToken });
  } catch (error) {
    // send error to email
    if (process.env.NODE_ENV === "production") {
      sendMail(
...
export const refreshToken = async (req, res) => {
  try {
    ...
    const remainingMs = new Date(user.sessionExpiresAt).getTime() - Date.now();
    ...
    return res.status(200).json({ _a_t: accessToken });
  } catch (error) {
    // send error to email
    if (process.env.NODE_ENV === "production") {
      sendMail(
