...
export const getAllOrders = async (req, res) => {
  ...
  try {
    const orders = await Order.find({})
      .select("orderId customer totalPrice orderDate orderStatus paymentStatus")
    ...
    const orders = await Order.aggregate([
      {
        $group: {
          ...
          confirmedOrders: {
            $sum: {
              $cond: [{ $eq: ["$orderStatus", "order confirmed"] }, 1, 0],
            },
          },
          completedOrders: {
            $sum: {
...
export const searchOrders = async (req, res) => {
  ...
  try {
    ...
    const orders = await Order.find({
      ...
      .select("orderId customer totalPrice orderStatus orderDate")
      .populate("customer", "firstName lastName")
      .skip((page - 1) * LIMIT)
      .limit(LIMIT);
